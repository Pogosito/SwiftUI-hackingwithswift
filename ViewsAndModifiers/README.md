...
Чтобы дать возможность нам размещать в ячейках более одного вью, требуется посавить модификатор ***@ViewBuilder*** перед переменной в инициализаторе, которая отвечает за кложуру
